...
Server.prototype = {
  ...
  config: function(key) {
    ...
    if (key !== 'range' && key !== 'static' && key !== 'staticMac' && key !== 'randomIP' && Options.opts[optId].enum) {
  ...
  _selectAddress: function(clientMAC, req) {
    ...
    const _staticOnly = this.config('staticIPOnly');
  ...
  handleDiscover: function(req) {
    ...
    const macFiltering = this.config('staticMac');
    if (typeof macFiltering === undefined || macFiltering.indexOf(req.chaddr) != -1)
    {
      const lease = this._state[req.chaddr] = this._state[req.chaddr] || new Lease;
      newIPAddress = this._selectAddress(req.chaddr, req);
      if (newIPAddress != null) {
        lease.address = newIPAddress;
  ...
  sendOffer: function(req) {
    ...
    if( this.config('behindDHCPRelay') ) {
      this._send(this.config('dhcpRelay'), ans, 67);
    } else {
  ...
  _send: function(host, data, port=CLIENT_PORT) {

    const sb = Protocol.format(data);

    this._sock.send(sb._data, 0, sb._w, port, host, function(err, bytes) {
      if (err) {
        console.log(err);
      } else {
